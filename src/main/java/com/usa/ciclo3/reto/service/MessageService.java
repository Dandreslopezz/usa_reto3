...
import com.usa.ciclo3.reto.repository.MessageRepository;
...
public class MessageService {
    ...
    public boolean deleteMessage(int id) {
        Boolean result = getMessage(id).map(message -> {messageRepository.delete(message);return true;}).orElse(false);
        return result;
    }
}